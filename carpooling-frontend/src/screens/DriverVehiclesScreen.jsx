...
import { getMyVehicles, addVehicle, deleteVehicle } from '../api/vehicles';

function VehicleCard({ v, onRemove, removing }) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--asphalt-900)', fontFamily: 'var(--font-mono)' }}>{v.vehicleNumber}</div>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <button
            ...
            disabled={removing}
            style={{ fontSize: 12, color: 'var(--danger-600)', background: 'none', border: 'none', cursor: removing ? 'wait' : 'pointer', fontFamily: 'var(--font-sans)', padding: 0, opacity: removing ? 0.5 : 1 }}
          >
...
export default function DriverVehiclesScreen() {
  ...
  const [form, setForm] = useState({ vehicleNumber: '', capacity: 4 });
  ...
  const handleAdd = async () => {
    const num = form.vehicleNumber.trim();
    if (!num) { setError('Vehicle number is required.'); return; }
    if (!form.capacity || form.capacity < 1) { setError('Capacity must be at least 1.'); return; }
    setError('');
    setSubmitting(true);
    try {
      const res = await addVehicle({ vehicleNumber: num, capacity: Number(form.capacity) });
      const created = res.data?.data;
      if (created) setVehicles(v => [...v, created]);
      else load();
      setForm({ vehicleNumber: '', capacity: 4 });
      setShowForm(false);
    } catch (err) {
      setError(err?.response?.data?.message || 'Failed to add vehicle. Plate may already exist.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', padding: 20, border: '1.5px dashed var(--asphalt-300)', boxShadow: 'var(--shadow-1)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div style={{ gridColumn: '1 / -1' }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--asphalt-500)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 5 }}>Vehicle number</label>
          <input
            value={form.vehicleNumber}
            onChange={e => set('vehicleNumber')(e.target.value.toUpperCase())}
            placeholder="MH 12 AB 1234"
            style={{ width: '100%', padding: '9px 12px', borderRadius: 'var(--radius-md)', border: '1.5px solid var(--asphalt-200)', fontSize: 13, fontFamily: 'var(--font-mono)', color: 'var(--asphalt-900)', outline: 'none', boxSizing: 'border-box', textTransform: 'uppercase' }}
          />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {[2, 3, 4, 5, 6, 7, 8].map(n => (
              <button key={n} onClick={() => set('capacity')(n)} style={{
                width: 36, height: 36, borderRadius: 'var(--radius-sm)', border: '1.5px solid',
                borderColor: form.capacity === n ? 'var(--ink-600)' : 'var(--asphalt-200)',
                background: form.capacity === n ? 'var(--ink-950)' : '#fff',
                color: form.capacity === n ? '#fff' : 'var(--asphalt-700)',
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <WpButton kind="accent" size="sm" onClick={handleAdd} disabled={submitting}>
          {submitting ? 'Adding…' : 'Add vehicle'}
        </WpButton>
        <WpButton kind="ghost" size="sm" onClick={() => { setShowForm(false); setError(''); }} disabled={submitting}>Cancel</WpButton>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {!loading && vehicles.length === 0 && !showForm ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {vehicles.map(v => <VehicleCard key={v.id} v={v} onRemove={handleRemove} removing={removingId === v.id} />)}
        </div>
      )}
    </div>
  );
  ...
  if (isDesktop) {
    const totalSeats = vehicles.reduce((s, v) => s + (v.capacity || 0), 0);
